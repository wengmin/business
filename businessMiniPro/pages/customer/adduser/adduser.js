...
Page({
  ...
  onShareAppMessage: function (res) {
    ...
    let that = this;
    util.request(api.CustomerUserSave, {
      photo: user.photo,
      ...
      weixin_openid: wx.getStorageSync('token'),
    }, 'POST').then(function (result) {
      if (result.errno === 0) {
        return {
          title: user.realname + '的名片',
          desc: '销擎名片管理',
          path: '/pages/card/index/index?userId=' + user.userId,
          imageUrl: user.photo,
        ...
        util.showErrorToast(result.errmsg);
      }
    });
  },
